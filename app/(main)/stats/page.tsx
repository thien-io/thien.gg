...
const achievements = [
  {
    ...
    desc: 'Reached Diamond in Valorant',
    icon: '💎',
    rare: 'Rare',
  },
  {
    title: 'The Completionist',
    desc: 'Finished Elden Ring 100%',
    icon: '👑',
    rare: 'Epic',
  },
  {
    title: 'Night Owl',
    desc: '500+ hours played after midnight',
    icon: '🦉',
    rare: 'Common',
  },
  {
    title: 'Veteran',
    desc: '5+ years active gaming',
    icon: '🎖️',
    rare: 'Rare',
  },
  {
    title: 'Speed Runner',
    desc: 'Sub-1h any% Hollow Knight',
    icon: '⚡',
    rare: 'Epic',
  },
  {
    title: 'Social Butterfly',
    desc: 'Gamed with 50+ different friends',
    icon: '🦋',
    rare: 'Common',
  },
];
...
function Counter({ to, suffix = '' }: { to: number; suffix?: string }) {
  ...
  return (
    <span ref={spanRef} style={{ display: 'inline' }}>
      <motion.span
        ...
        onUpdate={(v: number) => {
          if (spanRef.current) {
            spanRef.current.textContent = `${Math.round(v).toLocaleString()}${suffix}`;
          }
        }}
      />
    </span>
...
function AnimatedVBar({
  ...
  return (
    <div
      ref={ref}
      className='relative bg-muted/40 rounded-sm overflow-hidden flex-1'
