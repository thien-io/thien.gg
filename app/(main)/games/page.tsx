...
function GameCard({ game }: { game: Game }) {
  ...
  return (
    <Card className="group overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {game.rating && (
            <span className="flex items-center gap-1 text-xs text-muted-foreground">
              <Star className="w-3 h-3 text-idle fill-idle" />
              {game.rating}/10
            </span>
          )}
        </div>
...
export default function GamesPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-4xl font-bold font-display gradient-text">Games</h1>
        <p className="text-muted-foreground mt-1">
          {games.length} games · {totalHours.toLocaleString()} hours total
        </p>
      </div>
